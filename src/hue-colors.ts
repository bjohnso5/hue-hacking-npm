...
export class HueColors {
  ...
  private hexToGreen(hex: string): number {
    return Number.parseInt(hex.substring(2, 4), 16);
  }
  ...
  private hexToBlue(hex: string): number {
    return Number.parseInt(hex.substring(4, 6), 16);
  }
  ...
  private checkPointInLampsReach(p: XYPoint): boolean {
    ...
    const v2 = new XYPoint(CIEBlue.x - CIERed.x, CIEBlue.y - CIERed.y);
    const q = new XYPoint(p.x - CIERed.x, p.y - CIERed.y);
  ...
  private getClosestPointToPoint(xyPoint: XYPoint): XYPoint {
    ...
    const pAB = this.getClosestPointToLine(CIERed, CIELime, xyPoint);
    const pAC = this.getClosestPointToLine(CIEBlue, CIERed, xyPoint);
    const pBC = this.getClosestPointToLine(CIELime, CIEBlue, xyPoint);
  ...
  private getXYPointFromRGB(rgb: RGB): XYPoint {
    const r =
      rgb.r > 0.04045 ? (rgb.r + 0.055) / (1.0 + 0.055) ** 2.4 : rgb.r / 12.92;
    const g =
      rgb.g > 0.04045 ? (rgb.g + 0.055) / (1.0 + 0.055) ** 2.4 : rgb.g / 12.92;
    const b =
      rgb.b > 0.04045 ? (rgb.b + 0.055) / (1.0 + 0.055) ** 2.4 : rgb.b / 12.92;
    ...
    cx = Number.isNaN(cx) ? 0.0 : cx;
    cy = Number.isNaN(cy) ? 0.0 : cy;
    ...
    if (!inReachOfLamps) {
      const closestPoint = this.getClosestPointToPoint(xyPoint);
      cx = closestPoint.x;
      cy = closestPoint.y;
    }
  ...
  private getRGBFromXYAndBrightness(coords: XYPoint, bri: number): RGB {
    ...
    if (!this.checkPointInLampsReach(coords)) {
      ...
      updatedCoords = this.getClosestPointToPoint(coords);
    }

    // Calculate XYZ values Convert using the following formulas:
    const Y = bri;
    const X = (Y / updatedCoords.y) * updatedCoords.x;
    const Z = (Y / updatedCoords.y) * (1 - updatedCoords.x - updatedCoords.y);
  ...
  public getCIEColor(hexColor?: string): XYPoint {
    ...
    let xy: XYPoint = null;
    const lowerCaseHex =
      hex !== null ? hexColor.toString().toLowerCase() : null;
    const cssColor = this.cssColors.getHexCode(lowerCaseHex);
  ...
  public CIE1931ToHex(coords: XYPoint, bri?: number): string {
    let updatedBrightness = bri;
    if (bri === undefined) {
      updatedBrightness = 1;
    }
    const rgb = this.getRGBFromXYAndBrightness(coords, updatedBrightness);
